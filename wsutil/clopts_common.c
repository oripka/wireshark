...
static uint32_t max_selected = 0;
uint32_t max_packet_number = 0;
...
bool
add_selection(char *sel, uint *max_selection)
{
  char *locn;
  char *next;
...
int
selected_for_dissect(uint32_t recno)
{
...
void
parse_frame_range(const char *buf, const jsmntok_t *tokens, int count, struct select_item_range selections[], size_t maxlen, uint32_t *numselections)
{
    char *pch;
    uint32_t current = 0;
    uint32_t maxs = 0;
